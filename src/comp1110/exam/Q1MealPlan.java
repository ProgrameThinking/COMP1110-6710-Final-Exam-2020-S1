...
public class Q1MealPlan {
    ...
    private static boolean isVaildPlan(Food f1,Food f2) {
    	switch (f1.meal) {
    		case BREAKFAST:
    			return f2.meal==Meal.LUNCH;
    		case LUNCH:
    			return f2.meal==Meal.DINNER;
    		case DINNER:
    			return f2.meal==Meal.DESSERT;
    		case DESSERT:
    			return f2.meal==Meal.BREAKFAST;
    		default:
    			return false;
    	}
    }
    
    private static void generatePlans(Set<Food> foods, int numMeals, List<Food>meal , Set<List<Food>> plans) {
		if(meal.size()==numMeals) {
			plans.add(new ArrayList<>(meal));
			return;
		}
		for(Food food:foods) {
			if(!meal.contains(food)) {
				if(meal.isEmpty()||isVaildPlan(meal.get(meal.size()-1), food)) {
					meal.add(food);
					generatePlans(foods, numMeals, meal, plans);
					meal.remove(meal.size()-1);
				}
			}
		}
	}
    ...
    public static Set<List<Food>> getAllMealPlans(Set<Food> foods, int numMeals) {
        ...
        if (numMeals==0)
        	return new HashSet<>();
        Set<List<Food>> plans=new HashSet<>();
        generatePlans(foods,numMeals,new ArrayList<>(),plans );
        return plans;
    }
}
